...
import UserOne from '/src/images/user/default-user.png';
...
const UpdateProfileImage: React.FC = () => {
  const { userProfileInfo, setUserProfileInfo } = React.useContext(UniversalContext);
  ...
  const deleteProfileImage = async () => {
    try {
      ...
      const response = await fetch(`${serverAddress}/api/deleteprofileimg`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
      ...
      setUserProfileInfo({ ...userProfileInfo, profileImgUrl: null });
  ...
  const handlePhotoFormSubmit = (event: React.FormEvent<HTMLFormElement>) => {
          ...
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <span className="flex gap-2.5">
                <button type="button" onClick={deleteProfileImage} className="text-sm hover:text-primary">
                  Delete
                </button>
