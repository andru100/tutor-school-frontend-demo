...
const Settings: React.FC<Props> = ({ userProfileInfo, updateUserProfileInfo }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("useEffect getting adminUserData: ")
      ...
      const response = await fetch( serverAddress + "/api/query/getadminuser", {
        ...
        if (response.status === 200 ) {
          ...
          console.log("get adminUserData response is: ", data)
  ...
  if (adminUserData.dob) {
     formattedDobDate = new Date(adminUserData.dob).toISOString().slice(0, 16);
  }

  const UpdatePass = async (data: InfoRequest) => {
  

    console.log("sending update password or username: ", data)
    const accessToken = localStorage.getItem('accessToken') || null;
    ...
    if (response.ok) {
      ...
      console.log("error in update password, response is: ", errorData)
    }
  };

  
  const secureUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    try {
      console.log("sending signupdata: ", adminUserData);
      const accessToken = localStorage.getItem('accessToken') || null;
      const serverAddress = import.meta.env.VITE_APP_BACKEND_ADDRESS;
  
      const endpoint =
        userProfileInfo.role === 'Student'
      ...
      if (response.ok) {
        ...
        const errorMessage =
          userProfileInfo.role === 'Student'
            ...
            : "An error occurred while updating teacher data";
        toast.error(errorMessage);
        console.error(`Error updating ${userProfileInfo.role.toLowerCase()} data:`, errorData);
      }
    } catch (error) {
      const errorMessage =
        userProfileInfo.role === 'Student'
          ...
          : "An error occurred while updating teacher data";
      toast.error(errorMessage);
      console.error(`Error updating ${userProfileInfo.role.toLowerCase()} data:`, error);
    }
  };
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      const response = await fetch(`${serverAddress}/api/upload/ProfileImgUpload`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`
        },
        ...
        <div className="grid grid-cols-5 gap-8">
          <div className="col-span-5 xl:col-span-3">
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              ...
              <div className="p-7">
                <form action="#" onSubmit={secureUpdate}>
                  <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                    ...
                    <div className="w-full sm:w-1/2">
                      ...
                      <input
                        ...
                        placeholder="number"
                        onChange={handleInputChange}
                        value={adminUserData.phoneNumber  ?? ''}
                      />
                    </div>
                  </div>
                     ...
                     <div className="mb-5.5">
                      ...
                      <div className="relative">
                        <input
                          ...
                          defaultValue={adminUserData.accountNo  ?? ''}
                          onChange={handleInputChange}
                          className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                        />
                     ...
                     <div className="mb-5.5">
                      ...
                      <div className="relative">
                        <input
                          name='adjustments'
                          type="text"
                          value={adminUserData.adjustments  ?? ''}
                    ...
                    <div className="mb-5.5">
                      ...
                      <div className="relative">
                        ...
                        <textarea
                          ...
                          onChange={handleInputChange}
                          defaultValue={adminUserData.notes || ""}
                        ></textarea>
                      </div>
                    </div>
                ...
                <form action="#">
                  <div className="mb-4 flex items-center gap-3">
                    <div className="h-14 w-14 rounded-full">
                      <img src={userProfileInfo.profileImgUrl || UserOne} alt="User" />
                    </div>
                    <div>
                      ...
                      <span className="flex gap-2.5">
                        ...
                        <button className="text-sm hover:text-primary"  onClick={uploadFile}>
                          Update
                        </button>
                      </span>
                    </div>
