...
const UpcomingLessons: React.FC<Props> = ({lessons, backToParent}) => {
  ...
  const handleViewLesson = ( lesson: LessonEvent) => {
    navigate('/view-lesson', { state: { lesson, backToParent } });
  };
  ...
  const handleDelete = async (lessonId: number) => {
    try {
      ...
      const apiUrl = serverAddress + '/api/mutation/DeleteLesson';
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to delete lesson response is ' + JSON.stringify(response));
      }
          ...
          <tr className="bg-white dark:border-strokedark dark:bg-boxdark">
            ...
            <td className="py-5 px-4 dark:border-strokedark w-1/4">
              ...
              <>
                ...
                <button className="hover:text-primary" onClick={() => handleDelete(event.id)} title='Delete' >
                ...
                <button className="hover:text-primary" onClick={() => window.open(event.links, '_blank')} title="Join lessoon">
