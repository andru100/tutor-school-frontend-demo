...
const SignIn: React.FC = () => {
  ...
  const login = async (loginData: LoginData) => {
    try {
      ...
      if (!response.ok) {
        // custom message for login 404, handlefetch will check others
        if (response.status === 404) {
          console.error("User not found. Please check your login details.");
          toast.error("User not found. Please check your login details.");
          return;
        }
  ...
  const handleGoogleLogin  =   async (idToken: string) => {
    try {
        ...
        const response = await fetch( serverAddress + '/api/account/google-login', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        <div className="flex flex-wrap items-center">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              {/* <span className="mb-1.5 block font-medium">Title here</span> */}
              <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
                Sign In
              </h2>
              ...
              <button onClick={()=> googleSignIn()}className="flex w-full items-center justify-center gap-3.5 rounded-lg border border-stroke bg-gray p-4 hover:bg-opacity-50 dark:border-strokedark dark:bg-meta-4 dark:hover:bg-opacity-50">
                  <span>
                    <svg
                      width="20"
                      height="20"
                ...
                <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <button 
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center rounded-md border border-primary bg-primary px-6 py-2.5 text-center font-medium text-white hover:bg-opacity-90 transition-colors duration-200"
                  >
                    <span className="mr-2">
                  ...
                  <button 
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center rounded-md border border-stroke bg-gray px-6 py-2.5 text-center font-medium text-black dark:border-strokedark dark:bg-meta-4 dark:text-white hover:bg-opacity-90 transition-colors duration-200"
                  >
                    <span className="mr-2">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
