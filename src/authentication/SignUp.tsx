...
import EmailConfirm from './ConfirmEmail.tsx'
...
const SignUp: React.FC = () => {
  ...
  const signUp1 = async (Data: SignUpData) => {
    try {
  
      const response = await fetch(serverAddress + '/api/account/register', {
        method: 'POST',
        headers: {
  ...
  const handleGoogleSignUp = async (idToken: any) => {
    ...
    try{   
      ...
      const response = await fetch( serverAddress + '/api/account/registerGoogle', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.ok) {
        ...
        toast.success("Please select you account type")
        navigate('/create-role' , { state: { email: email } });
      } else if (response.status === 400) {
        toast.error("Error in  sign up. Please check that you have entered a valid email address and password. If you think you may have already registered, please try to sign in.")
        console.error("Error in signup, status code:", response.status)
      } else if (response.status === 404) {
        toast.error("Error: endpoint not found")
        console.error("Error endpoint not found, status code:", response.status)
      } else {
        toast.error("An error occurred during signup. Please try again. If the problem persists, please contact support")
        console.error("Error in signup, status code:", response.status)
      }
    } catch (error) {
      console.error("Error during signup:", error);
            ...
            <div className="flex flex-wrap items-center">
              ...
              <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                  ...
                  <button  onClick={() => login()} className="flex w-full items-center justify-center gap-3.5 rounded-lg border border-stroke bg-gray p-4 hover:bg-opacity-50 dark:border-strokedark dark:bg-meta-4 dark:hover:bg-opacity-50">
                      <span>
                        <svg
                          width="20"
                          height="20"
