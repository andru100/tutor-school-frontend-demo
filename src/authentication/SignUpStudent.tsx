...
import { Student, AdminCreateTeacherData} from "/src/dashboard/types.tsx";
...
const SignUpStudent: React.FC = () => {
  const { setRole, setStudentData}  = useContext(UniversalContext);
  ...
  const signUp = async (Data: AdminCreateTeacherData) => {
    try{
      ...
      const serverAddress = import.meta.env.VITE_APP_BACKEND_ADDRESS
        ...
        const response = await fetch( serverAddress + "/api/account/signupstudent", {
        method: 'POST',
        headers: {
